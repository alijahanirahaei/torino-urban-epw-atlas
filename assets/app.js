(() => {
  ...
  function makeIndex() {
    state.indexedFeatures = state.geojson.features.map((feature) => {
      ...
      return { feature, ring, bbox: [minLon, minLat, maxLon, maxLat] };
    });
  }
  ...
  function lookupFeature(lat, lon) {
    for (const item of state.indexedFeatures) {
      ...
      if (pointInRing(lon, lat, item.ring)) return item.feature;
    }
    return null;
  }
  ...
  function updateResult(props) {
    ...
    elements["status-badge"].textContent = readableStatus(props.status);
    elements["status-badge"].classList.toggle("provisional", props.edge || props.qa > 0 || !category);
    ...
    if (category) {
      ...
      elements["epw-download"].href = "#";
      elements["config-download"].href = "#";
  ...
  function performLookup(lat, lon, zoom = true) {
    ...
    if (zoom) state.map.setView([lat, lon], Math.max(state.map.getZoom(), 15));
  }
